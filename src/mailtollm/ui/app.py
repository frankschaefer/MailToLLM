...
class MailToLLMApp(ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        footer.grid_columnconfigure((1, 3, 5, 7, 9, 11), weight=1)
        ...
        ctk.CTkLabel(footer, text="Emails", text_color="#1f2a44").grid(
            row=0, column=6, padx=12, pady=8, sticky="w"
        )
        self.emails_label = ctk.CTkLabel(footer, text="0", text_color="#1f2a44")
        self.emails_label.grid(row=0, column=7, padx=12, pady=8, sticky="w")

        ctk.CTkLabel(footer, text="Attachments", text_color="#1f2a44").grid(
            row=0, column=8, padx=12, pady=8, sticky="w"
        )
        self.attachments_label = ctk.CTkLabel(footer, text="0", text_color="#1f2a44")
        self.attachments_label.grid(row=0, column=9, padx=12, pady=8, sticky="w")

        ctk.CTkLabel(footer, text="Contacts", text_color="#1f2a44").grid(
            row=0, column=10, padx=12, pady=8, sticky="w"
        )
        self.contacts_label = ctk.CTkLabel(footer, text="0", text_color="#1f2a44")
        self.contacts_label.grid(row=0, column=11, padx=12, pady=8, sticky="w")
    ...
    def _on_progress(self, processed: int, total: int, contacts: int = 0, emails: int = 0, attachments: int = 0) -> None:
        self._processed_records = processed
        self._total_records = total
        self._contact_count = contacts
        self._email_count = emails
        self._attachment_count = attachments
        # Update counters in UI
        self.after(0, lambda: self.contacts_label.configure(text=str(contacts)))
        self.after(0, lambda: self.emails_label.configure(text=str(emails)))
        self.after(0, lambda: self.attachments_label.configure(text=str(attachments)))
